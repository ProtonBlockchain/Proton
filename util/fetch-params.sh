...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/ProtonParams"
    else
        PARAMS_DIR="$HOME/.proton-params"
    fi
fi
...
Failed to fetch the Proton zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Proton - fetch-params.sh

This script will fetch the Proton zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Proton zkSNARK parameters. Note that it is
