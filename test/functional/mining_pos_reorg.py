...
class ReorgStakeTest(protonTestFramework):
    ...
    def check_money_supply(self, expected_pro):
        # verify that nodes have the expected PRO supply
        pro_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(pro_supply, [DecimalAmt(expected_pro)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that PRO supply was properly updated after the reorgs
        self.log.info("Check PRO supply...")
