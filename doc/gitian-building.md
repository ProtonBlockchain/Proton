...
*Setup instructions for a gitian build of Proton using a VM or physical system.*
...
to the Proton GitHub release page.
...
- [Building Proton](#building-proton-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/proton-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/proton-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/proton-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/proton-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/proton-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/proton-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/proton-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@proton.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/proton-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/proton-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/proton-osx-*-build.assert
```
