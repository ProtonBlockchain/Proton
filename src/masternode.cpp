...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 16050) return false;
    } else if (addr.GetPort() == 16050)
        return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int nChainHeight, int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 PRO tx got MASTERNODE_MIN_CONFIRMATIONS
...
{
    return std::make_pair(0, 2500 * COIN);
}
