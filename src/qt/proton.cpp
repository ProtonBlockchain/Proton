...
#include "qt/proton/splash.h"
#include "qt/proton/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("proton", psz).toStdString();
}
...
/** Class encapsulating Proton startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (ProtonGUI)
...
{
    window = new ProtonGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &ProtonGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &ProtonGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(ProtonGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(ProtonGUI::DEFAULT_WALLET);
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ProtonGUI::handlePaymentRequest);
        connect(window, &ProtonGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Proton can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Proton"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("Proton"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
