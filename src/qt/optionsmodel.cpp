...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::PRO);
    ...
    if (settings.contains("nAnonymizeprotonAmount") || reset)
        gArgs.SoftSetArg("-anonymizeprotonamount", settings.value("nAnonymizeprotonAmount").toString().toStdString());
