...
#include "qt/proton/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/proton/forms/ui_dashboardwidget.h"
#include "qt/proton/qtutils.h"
#include "qt/proton/sendconfirmdialog.h"
#include "qt/proton/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(ProtonGUI* parent) : PWidget(parent),
                                                      ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquarePro, "square-chart-pro");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelPro, "text-chart-pro");
    ...
    setCssProperty(ui->labelAmountPro, "text-stake-pro-disable");
    ...
    if (window)
        connect(window, &ProtonGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
const QMap<int, std::pair<qint64, qint64> > DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPro = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair PRO
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            pro = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalPro += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPro.append(pro);
        chartData->valuesMN.append(mn);

        int max = std::max(pro, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesPro);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalPro > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountPro, "text-stake-pro");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountPro, "text-stake-pro-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountPro, ui->labelAmountMN});
    ui->labelAmountPro->setText(GUIUtil::formatBalance(chartData->totalPro, nDisplayUnit));
