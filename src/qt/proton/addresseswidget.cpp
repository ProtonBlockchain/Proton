...
#include "qt/proton/forms/ui_addresseswidget.h"
#include "qt/proton/addresslabelrow.h"
#include "qt/proton/addnewaddressdialog.h"
#include "qt/proton/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(ProtonGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto proAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(proAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(proAdd);
        if (walletModel->updateAddressBookLabels(proAdd, label.toUtf8().constData(),
