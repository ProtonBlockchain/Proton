...
#include "qt/proton/forms/ui_snackbar.h"
#include "qt/proton/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ProtonGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ProtonGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
