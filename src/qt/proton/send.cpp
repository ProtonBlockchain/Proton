...
#include "qt/proton/send.h"
#include "qt/proton/forms/ui_send.h"
#include "qt/proton/addnewcontactdialog.h"
#include "qt/proton/qtutils.h"
#include "qt/proton/sendchangeaddressdialog.h"
#include "qt/proton/optionbutton.h"
#include "qt/proton/sendconfirmdialog.h"
#include "qt/proton/guitransactionsutils.h"
...
SendWidget::SendWidget(ProtonGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-pro");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onPROSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onPROSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent PROs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield PROs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent PROs to shield."));
    }
}
...
void SendWidget::onPROSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto proAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(proAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(proAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(proAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
