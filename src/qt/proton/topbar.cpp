...
#include "qt/proton/topbar.h"
#include "qt/proton/forms/ui_topbar.h"
#include "qt/proton/lockunlock.h"
#include "qt/proton/qtutils.h"
#include "qt/proton/receivedialog.h"
#include "qt/proton/loadingdialog.h"
#include "dashboardwidget.h"
#include "askpassphrasedialog.h"
#include "validation.h"

#include "bitcoinunits.h"
#include "qt/proton/balancebubble.h"
...
TopBar::TopBar(ProtonGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    //setCssProperty({ui->labelAmountTopPro}, "amount-small-topbar");
    setCssProperty({ui->labelAmountPro}, "amount-topbar");
    setCssProperty({ui->labelPendingPro, ui->labelImmaturePro, ui->labelAvailablePro, ui->labelLockedPro, ui->labelCollateralPro, ui->labelMasternodeCount,
    ...
    ui->pushButtonConf->setButtonText("proton.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open proton.conf with default application"));
}
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    ui->labelCollateralPro->setText(GUIUtil::formatBalance(CMasternode::GetMNCollateral(chainActive.Tip()->nHeight) * COIN, nDisplayUnit));
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // Top
    ui->labelAmountTopPro->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    // Expanded
    ui->labelAmountPro->setText(GUIUtil::formatBalance(newBalance.balance + newBalance.immature_balance, nDisplayUnit));
    ui->labelAvailablePro->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    ui->labelPendingPro->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmaturePro->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
    ui->labelLockedPro->setText(GUIUtil::formatBalance(nLockedBalance, nDisplayUnit));
