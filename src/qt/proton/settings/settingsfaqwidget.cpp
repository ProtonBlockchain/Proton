...
#include "qt/proton/settings/settingsfaqwidget.h"
#include "qt/proton/qtutils.h"
#include "qt/proton/settings/forms/ui_settingsfaqwidget.h"
#include <QMetaObject>
#include <QScrollBar>

SettingsFaqWidget::SettingsFaqWidget(ProtonGUI* parent) : QDialog(parent),
                                                                           ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({ui->labelSubtitle_Intro,
                       ui->labelSubtitle_UnspendablePRO,
    ...
    setCssProperty({ui->labelContent_Intro,
                       ui->labelContent_UnspendablePRO,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("Proton is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. Proton incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("Proton utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of Proton to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendablePROContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received Proton requires 6 confirmations on the network "
               "to become eligible for spending which can take ~12-15 minutes.")) +
        formatFAQParagraph(
            tr("Your Proton wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendablePRO->setText(unspendablePROContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of Proton with a minimum of 60 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("proton", "<span style=\"font-style:italic\">getstakingstatus</span>")));

    ui->labelContent_Stake->setText(stakeContent);

    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("For Support you can contact us on our Email: info@protonnodes.com")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node Proton wallet with a "
               "requirement of Collateral Amount of PRO secured collateral to provide extra services "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in Proton Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the Proton network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Collateral Amount of PRO per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the Collateral Amount of PRO collateral "
    ...
    connect(ui->pushButton_UnspendablePRO, &QPushButton::clicked, [this]() { onFaqClicked(ui->widget_UnspendablePRO); });
    ...
    if (parent)
        connect(parent, &ProtonGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
