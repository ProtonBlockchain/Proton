...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("ProtonMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("ProtonMiner : ProcessNewBlock, block not accepted");
    }
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    util::ThreadRename("proton-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ProtonMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ProtonMiner exception");
    } catch (...) {
        LogPrintf("ProtonMiner exception");
    }

    LogPrintf("ProtonMiner exiting\n");
}
