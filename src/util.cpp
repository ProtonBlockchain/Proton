...
const char * const Proton_CONF_FILENAME = "proton.conf";
const char * const Proton_PID_FILENAME = "proton.pid";
const char * const Proton_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Proton
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Proton
// Mac: ~/Library/Application Support/Proton
// Unix: ~/.proton
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Proton";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ProtonParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ProtonParams
    // Mac: ~/Library/Application Support/ProtonParams
    // Unix: ~/.proton-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ProtonParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "ProtonParams";
#else
    // Unix
    return pathRet / ".proton-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/proton/sapling-spend.params";
        sapling_output = "/usr/share/proton/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/proton/sapling-spend.params";
            sapling_output = "/usr/local/share/proton/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", Proton_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", Proton_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty proton.conf if it does not exist
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", Proton_PID_FILENAME));
