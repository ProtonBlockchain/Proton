...
CProStake* CProStake::NewProStake(const CTxIn& txin)
{
    ...
    return new CProStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CProStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CProStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CProStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CProStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CProStake::GetUniqueness() const
{
    //The unique identifier for a PRO stake is the outpoint
...
const CBlockIndex* CProStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CProStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CProStake::ContextCheck(int nHeight, uint32_t nTime)
{
