...
#ifndef Proton_STAKEINPUT_H
#define Proton_STAKEINPUT_H
...
class CProStake : public CStakeInput
{
private:
...
public:
    CProStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CProStake* NewProStake(const CTxIn& txin);
