...
#if defined(NDEBUG)
#error "Proton cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in upro) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("proton-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> proInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                proInputs.push_back(stakeIn);
        }
        const bool hasPROInputs = !proInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasPROInputs) {
                    ...
                    for (const CTxIn& proIn : proInputs)
                        if(proIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasPROInputs) {
                            for (const CTxIn& stakeIn : proInputs)
